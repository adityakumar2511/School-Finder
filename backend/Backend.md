...
└── src/
    ...
    │   ├── mailer.ts          # Resend; sendOtpEmail unused by auth
...
| Forgot password | `POST /api/auth/forgot-password` | Email + optional `expectedRole`; Resend link; always 200 generic |
| Reset password | `POST /api/auth/reset-password` | Token + optional `expectedRole` |
...
| `RESEND_API_KEY` | Warn if missing | Password reset email |
...
| `mailer.ts` | Resend password reset; `sendOtpEmail` **unused** |
...
| `mailer.sendOtpEmail` | **Unused** — OTP is SMS via Fast2SMS |
