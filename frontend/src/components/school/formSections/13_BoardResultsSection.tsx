    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="font-body text-body text-gray-400 mt-1">
          Year-wise board exam performance — Class 10 & Class 12
        </p>
      </div>
      ...
      <div className="space-y-4">
              ...
              <CardContent className="p-6 space-y-4">
                <div className="flex items-center justify-between">
                  <p className="font-heading text-label font-semibold text-gray-700">
                    Result #{index + 1} — {watch(`boardResults.results.${index}.year`) || "Year"}
                  </p>
                  <Button
                    type="button"
                    variant="ghost"
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  <div className="space-y-1.5">
                    <Label className="font-heading text-label text-gray-800">Year</Label>
                    <Input
                      type="number"
                      min={2000}
                      max={CURRENT_YEAR}
                  ...
                  <div className="space-y-1.5">
                    <Label className="font-heading text-label text-gray-800">Class 10 Pass %</Label>
                    <Input
                      type="number"
                      min={0}
                      max={100}
                      placeholder="e.g. 98"
                      className={inputClass}
                      {...register(`boardResults.results.${index}.class10Percent`)}
                    />
                  </div>
                  <div className="space-y-1.5">
                    <Label className="font-heading text-label text-gray-800">Class 12 Pass %</Label>
                    <Input
                      type="number"
                      min={0}
                      max={100}
                      placeholder="e.g. 96"
                      className={inputClass}
                      {...register(`boardResults.results.${index}.class12Percent`)}
                    />
                  </div>
                  <div className="space-y-1.5">
                    <Label className="font-heading text-label text-gray-800">Topper Name</Label>
                    <Input
                  ...
                  <div className="space-y-1.5">
                    <Label className="font-heading text-label text-gray-800">Top Score</Label>
                    <Input
        ...
        >
          <Plus className="w-4 h-4 mr-2" /> Add Year Result
        </Button>
      </div>
        ...
        <CardContent className="p-6 space-y-4">
          ...
          {customFields.length === 0 && (
            <p className="font-body text-meta text-gray-400 text-center py-3">
              Add any extra result details — district rank, distinctions, etc.
            </p>
          )}
